...
tgui::String WordDetail::getDefinition(tgui::String& str) {
	return this->dataExec->getDefinition(str);
}

void WordDetail::setOnClickEdit() {
...
void WordDetail::initAcceptedBtn() {
...
WordDetail::WordDetail(tgui::Gui* GUI, int x, int y, int w, int h, tgui::String str) : x(x), y(y), w(w), h(h) {
	this->favorite = false;
	this->gui = GUI;
...
void WordDetail::changeWord(const tgui::String& str) {
